# Service for importing New Zealand neighborhood boundaries from GADM ADM3 dataset
# Filters suburbs by NAME_2 (city/territorial authority) from the national dataset
#
# Usage:
...
class NewZealandNeighborhoodImporter
  ...
  def initialize(city_key)
    ...
    unless parent_name_field.present? || bounding_box.present?
      raise ArgumentError,
            "City '#{city_key}' is not configured for New Zealand (missing parent_name field mapping or bounding box filter)"
    end
  end

  # Import neighborhoods for the configured New Zealand city
  # Returns the number of neighborhoods imported
  def import_neighborhoods
    Rails.logger.info "Importing New Zealand suburbs for #{city_name}"
  ...
  # Check if a city is configured for New Zealand import
  def self.available_for_city?(city_key)
    ...
    return false unless city_config &&
                        city_config["country"] == "New Zealand" &&
  ...
  # Get list of all New Zealand cities
  def self.available_cities
    config = YAML.load_file(Rails.root.join("config", "neighborhood_boundaries.yml"))
    config.select { |_, v| v["country"] == "New Zealand" && v["enabled"] != false }.keys
  rescue
    []
  end
  ...
  # Get the parent name field for filtering (NAME_2 for territorial authority in GADM)
  def parent_name_field
    @parent_name_field ||= config.dig("field_mappings", "parent_name")
  end

  # Get expected parent city/territorial authority name(s) for filtering
  ...
  def expected_parent_names
    @expected_parent_names ||= begin
      ...
      case city_key
      when 'auckland'
        names << 'Auckland' << 'Auckland City'
      when 'wellington'
        names << 'Wellington' << 'Wellington City'
      when 'christchurch'
        names << 'Christchurch' << 'Christchurch City'
      end
  ...
  def fetch_and_filter_features
    ...
    begin
      Rails.logger.info "Fetching New Zealand national GeoJSON from #{url}"
      ...
      unless response.success?
        Rails.logger.error "New Zealand GeoJSON error: #{response.status} - #{response.body[0..200]}"
        return []
      end
      ...
      Rails.logger.info "Fetched #{all_features.size} total New Zealand suburbs"
      ...
      Rails.logger.error "Error fetching New Zealand neighborhoods: #{e.message}"
  ...
  def filter_features_by_parent(all_features)
    ...
    Rails.logger.info "Filtered to #{filtered_features.size} suburbs for #{city_name}"
  ...
  def import_neighborhood(feature)
    ...
    geoid = "NZ_#{raw_geoid}"  # Prefix with NZ_ to distinguish from other countries
