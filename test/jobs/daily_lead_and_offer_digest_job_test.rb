...
class DailyLeadAndOfferDigestJobTest < ActiveJob::TestCase
  ...
  test "notifies hosts of new matching searches created in last 24 hours" do
    ...
    assert_equal [@host1.email], email.to
    assert_match /new.*looking in your area/i, email.subject
  end
  ...
  test "only notifies hosts with recurring subscriptions" do
    ...
    assert_not_includes recipients, @lifetime_host.email
  end
  ...
  test "matches searches by rating (min_rating)" do
    ...
    # But only host1 and host2 get emails (recurring subscribers)
    emails = ActionMailer::Base.deliveries
    recipients = emails.flat_map(&:to).uniq

    assert_includes recipients, @host1.email
    assert_includes recipients, @host2.email
    assert_not_includes recipients, @lifetime_host.email # No recurring subscription
  end
